...
- Two connection modes: **polling** (default, no public URL needed) and **webhook** (for production with public URL)
...
- Always push to remote after successful commit (regardless of branch strategy)
...
- **Observability**: Structured JSON logging (pino) for all requests, Claude interactions, git operations, and deploy events.
- **Resilience**: If Claude fails mid-task, rollback uncommitted changes and notify user.
- **Graceful Shutdown**: SIGINT/SIGTERM handlers stop the queue, bot, and server cleanly.
